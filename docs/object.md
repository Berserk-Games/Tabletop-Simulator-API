...
<a class="anchor" id="gettable"></a>getTable([<span class="tag str"></span>](types.md)&nbsp;table_name) | Data value of a variable in another Object's script. Can only return a table. Updating the table returned does not change the value that is returned in subsequent calls to getTable, that table must be updated and setTable|[#settable] must be called.| [<span class="ret tab"></span>](types.md) |
